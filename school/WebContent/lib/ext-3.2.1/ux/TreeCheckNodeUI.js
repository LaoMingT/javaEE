Ext.ux.TreeCheckNodeUI = function(conf) {
	// 多选: 'multiple'(默认)
	// 单选: 'single'
	// 级联多选:
	// 'cascade'(同时选父和子);'parentCascade'(选父);'childCascade'(选子)
	// 'cascadeNotExpand' (展开时不加载结点);
	this.checkModel = 'cascadeNoLoad';
...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		// add some indent caching, this helps performance
		// when rendering a large tree
	...
	check : function(checked) {
		...
		if (this.checkModel == 'single') {
			...
			if (this.checkModel == 'cascadeNoLoad') {
				this.childCheck(n);
			}
		}
	},
