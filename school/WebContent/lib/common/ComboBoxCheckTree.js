...
Ext.extend(Ext.ux.ComboBoxCheckTree, Ext.form.ComboBox,
		{
			...
			separator : ',',
			initEvents : function() {
				Ext.ux.ComboBoxCheckTree.superclass.initEvents.apply(this,
						arguments);
			...
			setValue : function(ids, names) {
				if (null != ids && '' != ids) {
					this.value = ids;
					this.setRawValue(names);
					return;
				}
				var values = [];
				var texts = [];
				...
				for ( var i = 0; i < checkedNodes.length; i++) {
					...
					if (this.selectValueModel == 'all'
							|| (this.selectValueModel == 'leaf' && node
									.isLeaf())
							|| (this.selectValueModel == 'folder' && !node
									.isLeaf())) {
						values.push(node.id);
