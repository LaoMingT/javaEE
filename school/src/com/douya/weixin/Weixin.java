...
public class Weixin {
    ...
    public List<Fan>getWeixinFansList()
    {
    	return fans;
    }
	...
	private boolean _login() {
		try {
			...
			System.out.println("登录密码加密后:"+DigestUtils.md5Hex(this.loginPwd
					.getBytes()));
			...
			if (status == HttpStatus.SC_OK) {
				...
				System.out.println("获取返回信息"+ret+retcode.getRet()+ret);
				...
				if (retcode.getRet() == 0 && retcode.getErrCode() == 0) {
					...
					this.isLogin = true;
					this.token = getTokenFromRet(ret);//getToken(retcode.getErrMsg());
					return true;
				}
	...
	/**
	 * 从登录成功返回String信息中分离出token信息
	 *
	 * @param ret
	 * @return
	 */
    private String getTokenFromRet(String ret)
    {
    	return ret.substring(ret.indexOf("token=")+6,ret.indexOf("\"",ret.indexOf("token="))).trim();
    }
	/**
	 * 从登录成功的信息中分离出token信息
	 *
	...
	public int getFans() {
		try {
			...
			System.out.println("获取粉丝请求链接"+CONTACT_MANAGE_PAGE + paramStr);
...
//					System.out.println(get.getResponseBodyAsString());
	 ...
	 * @date 2013-11-20
	 */
	public boolean sendMsg(String fanId,String mes)
	{
		try {
			if (!this.isLogin) {
				this._login();
			...
			if (this.isLogin) {
				...
				post.setHeader(REFERER_H,"https://mp.weixin.qq.com/cgi-bin/singlesendpage?t=message/send&action=index&tofakeid="+fanId+"&token="+this.token+"&lang=zh_CN");
				...
				formParams.add(new BasicNameValuePair("content", mes)); //说话内容
	...
	public static void main(String[] args) {
		...
		String LOGIN_USER = "java_php_wmb@126.com"; //此为上一任作者的用户名和密码，截止到我最后用发现还能用
		String LOGIN_PWD = "wangmingbo";
		Weixin wx = new Weixin(LOGIN_USER, LOGIN_PWD);
		wx.login();
		wx.getCookiestr();
		System.out.println("cookie:"+wx.getCookiestr());
		//ImgFileForm form = new ImgFileForm();
		//form.setUploadfile(new File("D:\\Data\\image\\4.jpg"));
		// wx.updateImg(form);
		System.out.println("粉丝数："+wx.getFans());
	//	wx.sendMsg(,"helloworld");//像好友列表中的第几个好友发消息，从0开始
